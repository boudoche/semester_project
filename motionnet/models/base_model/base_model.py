...
class BaseModel(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        if self.config.get('eval_waymo',False):
            ...
            json.dump(self.pred_dicts, open(os.path.join(self.config['nuscenes_submission'], 'results', "evalai_submission.json"), "w"))
    ...
    def log_info(self, batch, prediction, status='train'):
        ...
        if status == 'val' and self.config.get('eval', False):
            ...
            new_dict = {}
            kalman_difficulties = inputs["kalman_difficulty"][:, -1].cpu().numpy() # Last is difficulty at 6s (others are 2s and 4s)
