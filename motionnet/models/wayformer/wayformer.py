...
class Wayformer(BaseModel):
    ...
    def process_observations(self, ego, agents):
        ...
        env_masks = env_masks.unsqueeze(1).repeat(1, self.num_queries_dec, 1).view(ego.shape[0] * self.num_queries_dec, -1)
    ...
    def _forward(self, inputs):
        ...
        road_pts_feats = self.selu(self.road_pts_lin(roads[:, :self.max_num_roads, :, :self.map_attr]).view(B,-1,self.d_k)) + self.map_positional_embedding
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.0002, steps_per_epoch=1, epochs=150, pct_start=0.02, div_factor=100.0, final_div_factor=10)
...
class Criterion(nn.Module):
    ...
    def forward(self, out, gt, center_gt_final_valid_idx):
        return self.nll_loss_gmm_direct(out['predicted_probability'], out['predicted_trajectory'], gt, center_gt_final_valid_idx)

    def nll_loss_gmm_direct(self, pred_scores, pred_trajs, gt_trajs, center_gt_final_valid_idx, pre_nearest_mode_idxs=None,
